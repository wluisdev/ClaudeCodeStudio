...
- **Plan approval modal now displays Markdown, opens wider, and is resizable** ([#17](https://github.com/wluisdev/ClaudeCodeStudio/issues/17)). When approving a plan in plan mode, the dialog now renders the plan with full Markdown formatting (headings, lists, code blocks, task lists) instead of plain text, and the content area grows to fit taller plans. The plan modal opens wider by default and can be dragged to any width to read more; long tool inputs (like a subagent prompt) are draggable too, while short permission prompts stay compact. The plan font follows the chat's Ctrl+Scroll size, so the whole plan zooms in or out proportionally.

### Fixed

- **Clicking a file link in a plan navigated the whole panel to a "file not found" page with no way back.** The plan modal renders Markdown, and a link in it used to load in the panel itself, replacing the chat with a dead error page. File links in a plan now open in the Visual Studio editor (like the ones in chat), external links open in the browser, and a navigation guard keeps the panel from ever leaving the app.
